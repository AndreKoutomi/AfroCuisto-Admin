...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-bg-base text-[#2B3674] selection:bg-brand selection:text-white font-dm">
      {/* Venus Sidebar */}
      <aside className="w-[280px] bg-white flex flex-col pt-10 px-6 shrink-0 z-10 shadow-[4px_0_24px_rgba(112,144,176,0.05)]">
        <div className="flex items-center gap-3 mb-10 pb-6 border-b border-[#E0E5F2]">
          <div className="text-[#2B3674] font-bold text-2xl tracking-tight flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-brand flex items-center justify-center">
              <ChefHat className="text-white" size={20} />
            </div>
            AFRO<span className="font-extrabold text-brand">CUISTO</span>
          </div>
        </div>
        ...
        <nav className="space-y-3">
          {[
            ...
            <button
              ...
              className={`sidebar-item w-full flex items-center gap-4 px-2 py-3 ${activeTab === item.id ? 'active' : ''
                }`}
            >
              <item.icon size={22} className={activeTab === item.id ? 'text-brand' : 'text-[#A3AED0]'} />
              <span className="text-[16px] pr-2 tracking-wide text-left flex-1">{item.label}</span>
            </button>
          ))}
        </nav>
        ...
        <div className="mt-auto pb-10 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-br from-[#868CFF] to-brand text-white relative overflow-hidden shadow-lg">
          ...
          <button className="sidebar-item flex items-center gap-4 px-2 py-3 w-full transition-all">
            <LogOut size={22} className="text-[#A3AED0]" />
            <span className="text-[16px] tracking-wide font-medium">Log out</span>
          </button>
        </div>
      </aside>

      {/* Main Container */}
      <div className="flex-1 flex flex-col h-full bg-[#f4f7fe]">
        <header className="h-[96px] bg-[#f4f7fe]/80 backdrop-blur-xl flex items-center justify-between px-8 z-50 sticky top-0 transition-all">
          ...
          <div className="flex items-center gap-4 bg-white rounded-full p-2 shadow-[14px_17px_40px_4px_rgba(112,144,176,0.08)]">
            <div className="flex items-center gap-2 bg-[#F4F7FE] rounded-full px-5 py-3 flex-1 min-w-[214px]">
              <Search size={18} className="text-[#2B3674]" />
              <input
                type="text"
                placeholder="Search..."
                className="bg-transparent border-none p-0 text-sm font-medium focus:ring-0 text-[#2B3674] placeholder:text-[#A3AED0] w-full"
                value={searchQuery}
            ...
            <div className="flex items-center gap-4 px-2">
              ...
              <button className="w-10 h-10 rounded-full overflow-hidden border-2 border-transparent hover:border-brand transition-colors">
          ...
          <div className="max-w-[1500px] mx-auto">
            {activeTab === 'recipes' && (
              <div className="mb-8 flex justify-end">
                <button
                  onClick={addNewDish}
                  className="bg-brand text-white px-6 py-3.5 rounded-[16px] flex items-center gap-2 font-bold text-[14px] hover:bg-brand/90 transition-all shadow-[14px_17px_40px_4px_rgba(67,24,255,0.18)] translate-y-[-2px] hover:translate-y-0"
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredRecipes.map((recipe, idx) => (
                      <motion.div
                        ...
                        key={recipe.id}
                        className="venus-card flex flex-col p-[18px] group cursor-pointer"
                        onClick={() => {
                      ...
                      >
                        ...
                        <div className="flex-1 flex flex-col">
                          ...
                          <div className="mt-auto flex items-center justify-between pt-2">
                            ...
                            <button className="bg-[#E9E3FF] text-brand px-4 py-2 rounded-full font-bold text-[14px] group-hover:bg-brand group-hover:text-white transition-colors">
                              Edit
                            </button>
                          </div>
                        </div>
